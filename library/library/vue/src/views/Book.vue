  ...
  <div class="home" style="padding: 10px">
    ...
    <el-table :data="tableData" stripe border="true" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="status" label="状态">
        <!--   前端用0表示图书已借阅,所有后端数据库采用0表示图书未归还
          以下包含判断图书状态,0是已借阅,1是未借阅
          -->
      ...
      <el-table-column fixed="right" label="操作">
        <template v-slot="scope">
          ...
          <!-- status不等于1时，用户才能点击这个按钮来执行还书操作    -->
          <el-popconfirm title="确认还书?"
                         @confirm="handlereturn(scope.row.id,scope.row.isbn,scope.row.borrownum,user.id)"
                         v-if="user.role == 2" :disabled="scope.row.status == 1">
            <template #reference>
              <el-button type="danger" size="mini" :disabled="scope.row.status == 1">还书</el-button>
            </template>
          </el-popconfirm>
        </template>
      </el-table-column>
    ...
    <div style="margin: 10px 0">
      ...
      <!--上架图书,通过:model与data()中的form进行数据绑定
          通过dialogVisible控制弹窗的显示与隐藏,默认是false
          -->
      ...
      <!--点击修改图书信息后的数据回显功能,通过:model与data()中的form进行数据绑定
          通过dialogVisible控制弹窗的显示与隐藏,默认是false-->
...
export default {
  ...
  methods: {
    ...
    load() {
      ...
      request.get("/book", {
        params: {
          ...
          search2: this.search2,//name
          search3: this.search3,//author
        }
      }).then(res => {
        console.log(res)
     ...
     * 还书方法
     * */
    handlereturn(id, isbn, bn, userId) {
      /*(this.isbnArray.indexOf(scope.row.isbn)) == -1
      ...
      //id是图书的id
      this.form.status = "1"//更新图书状态为1,表示已归还

      //ElMessage.success("userId"+userId)
      this.form.isbn=isbn
      this.form.id=id
      request.put(`/book/return?id=${userId}`, this.form).then(res => {
        console.log(res)
        if (res.code == 200) {
          ElMessage({
            message: '还书成功',
    ...
    handlelend(id, isbn, name, bn) {
      ...
      this.load()
      this.form.status = "0"//更新图书状态为0,表示借出
      this.form.id = id
      this.form.borrownum = bn + 1
      console.log(bn)
      request.put("/book/lend", this.form).then(res => {
        console.log(res)
        if (res.code == '200') {
